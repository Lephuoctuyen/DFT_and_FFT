...
Trong bài báo này chúng sẽ thảo luận về thuật cho phép nhân 2 đa thức với kích thức là $n$ với thời gian $O(n \log n)$, điều này rất hơn khi nhân nhân có thời gian $O(n^2)$. 
...
We divide it into two smaller polynomials, the one containing only the coefficients of the even positions, and the one containing the coefficients of the odd positions:
 Chúng sẽ chia thành 2 đa thưc nhỏ, mỗi đa thức chỉ có hiệu số tại trí chẵn và lẻ. 
$$\begin{align}
